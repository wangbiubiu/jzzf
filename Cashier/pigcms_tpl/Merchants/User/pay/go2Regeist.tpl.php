...
$(".js_pic_url,.js_pic_url .icon20_common add_gray").dropzone({
    url: "?m=User&c=pay&a=uploadcertImg",
...
$("#classification_three>option").click(function () {
    ...
    <script type="text/javascript">
        function getnextLevel() {
            ...
            $.post('?m=User&c=pay&a=getSecondLevel', {id: provinceid}, function (ret) {
                if (ret.data) {
        ...
        function GetDistrict() {
            ...
            $.post('?m=User&c=pay&a=getThirdLevel', {id: cityid}, function (ret) {
                if (ret.data) {
                    $.each(ret.data, function (nn, vv) {
                        console.log(vv);
                        cityHtml += '<option value="' + vv.id + '" data-fullname="' + vv.name + '"  >' + vv.name + '</option>';
        ...
        function GetCircle() {
            ...
            $.ajax({
                type: 'post',
                url: '?m=User&c=pay&a=getFourthlevel',
                dataType: 'json',
                data: {id: obj},
                success: function (ev) {
