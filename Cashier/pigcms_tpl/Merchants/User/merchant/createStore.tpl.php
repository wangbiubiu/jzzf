				 ...
				 <div class="frm_control_row"> 
				  ...
				  <div class="frm_controls input_controls" style="margin:15px 0 0 5px"> 
				   <div class="input_btn group" > 
					详细地址：<span class="frm_input_box"> <input placeholder="输入详细地址，请勿重复填写省市区信息" name="address" id="searchSubmit" class="frm_input" type="text" />
					</span> 
...
$('#sub_add_shop').click(function(){
		...
		$.ajax({
			url:'?m=User&c=merchant&a=addShop',
			type:"post",
			data:$('form').serialize(),
			dataType:"JSON",
...
qq.maps.event.addDomListener(btnSearch, 'click', function () {
    ...
    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + add + ",0)&page_size=9&page_index=1&key=S6PBZ-D7BRQ-BNB5S-G2LBZ-PYAIO-DJF4K&output=jsonp&&callback=?");
