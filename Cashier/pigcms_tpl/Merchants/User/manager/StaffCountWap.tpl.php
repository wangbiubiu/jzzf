...
<div style="width: 100%;height: 50px;line-height: 50px;border-bottom: 1px solid #efefef;background-color: #008fd3;">
    ...
    <a href="/merchants.php?m=User&c=manager&a=index" onclick="" style="padding:0 20px 0 10px;display: block;float:left;color:#fff;">< 返回</a>
    <a href="/merchants.php?m=User&c=manager&a=CashierCount" onclick="" style="color:#fff;padding:0 10px 0 20px;display: block;float:right;"><?php echo $storname; ?></a>
</div>
<div style="padding:15px 10px;border-bottom: 1px solid #efefef;">
    <?php  if($getdata['type']=='weixin'){?>
        <div style="width: 49%;float:left;text-align: center;border-right: 1px solid #efefef;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$weixin['count']);?></div>
            <div style="color:#999;">收款金额（元）</div>
        </div>
        <div style="width: 49%;float:right;text-align: center;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$weixin['income']);?></div>
            <div style="color:#999;">实收金额（元）</div>
        </div>
    <?php  }elseif($getdata['type']=='alipay'){?>
        <div style="width: 49%;float:left;text-align: center;border-right: 1px solid #efefef;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$alipay['count']);?></div>
            <div style="color:#999;">收款金额（元）</div>
        </div>
        <div style="width: 49%;float:right;text-align: center;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$alipay['income']);?></div>
            <div style="color:#999;">实收金额（元）</div>
        </div>
    <?php  }else{?>
        <div style="width: 49%;float:left;text-align: center;border-right: 1px solid #efefef;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$total['count']);?></div>
            <div style="color:#999;">收款金额（元）</div>
        </div>
        <div style="width: 49%;float:right;text-align: center;">
            <div style="margin-bottom: 5px;font-size: 24px;color:#008fd3;"><?php echo sprintf("%.2f",$total['income']);?></div>
            <div style="color:#999;">实收金额（元）</div>
        </div>
    <?php  }?>
    ...
    <form method="get" action="/merchants.php?m=User&c=manager&a=StaffCount">
        ...
        <div style="padding: 10px 0;">
            ...
            <div style="width: 80%;float:right;">
                <input type="text" readonly="readonly" value="<?php if(isset($getdata['start'])) {echo $getdata['start'];}else{echo date('Y-m-d',time());}?>" name="start" class="input-sm form-control" id="datestart" placeholder="开始时间" style="text-indent: 0;text-align: center; padding:5px 10px;margin-bottom: 0px; width: 40%;border: 1px solid #efefef;height: 30px;display: inline-block;">
                <span style="width: 15%;display: inline-block;text-align: center;">到</span>
                <input type="text" readonly="readonly" value="<?php if(isset($getdata['end'])) {echo $getdata['end'];}else{echo date('Y-m-d',time());}?>" name="end" class="input-sm form-control" id="dateend" placeholder="结束时间" style="text-indent: 0;text-align: center; padding:5px 10px;margin-bottom: 0px; width: 40%;border: 1px solid #efefef;height: 30px;display: inline-block;">
            </div>
            <div style="clear: both;"></div>
        </div>
        ...
        if(!empty($neworder)){
            foreach($neworder as $v){
                ...
                <tr style="height: 40px;">
                    ...
                    <td style="font-size: 12px;"><?php echo date('Y-m-d',$v['paytime'])."<br/>".date('H:i:s',$v['paytime']);?></td>
