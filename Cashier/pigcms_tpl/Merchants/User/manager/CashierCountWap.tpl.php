...
<head lang="en">
    ...
    <style type="text/css">
        ...
        input[type='text']{width: 100%;height: 46px;border: none;text-indent: 10px;padding: 0;}
    </style>
</head>
<body>
<div style="padding: 10px;height: 60px;">
    <form method="get" action="/merchants.php?m=User&c=manager&a=CashierCount">
        <input type="hidden" value="User" name="m">
        <input type="hidden" value="manager" name="c">
        <input type="hidden" value="CashierCount" name="a">
        <div style="width: 90%;float:left;">
            <input class="" type="text" name="username" placeholder="输入收银员名称" value="<?php if(isset($getdata['username'])) echo $getdata['username'];?>" style="border-radius: 5px;border: none;">
        </div>
        <div style="width: 10%;float:right;text-align: center;">
            <input type="submit" value="搜索" style="border: none;background: none;height:50px;"/>
        </div>
    </form>
</div>
<a href="/merchants.php?m=User&c=manager&a=ShopownerCount" style="width: 100%;height: 50px;line-height:50px;text-indent:10px;color:#333;background-color: #fff;border-top: 1px solid #eee;border-bottom: 1px solid #eee;margin-top: 10px;display: block;">
    全部收银员
</a>
<?php
if(!empty($store)){
    foreach($store as $k=>$v){ ?>
        <a href="/merchants.php?m=User&c=manager&a=StaffCount&id=<?php echo $v['eid']?>" style="width: 100%;height: 50px;line-height:50px;text-indent:10px;color:#333;background-color: #fff;border-top: 1px solid #eee;border-bottom: 1px solid #eee;margin-top: 10px;display: block;">
            <?php echo  $v['username']."&nbsp;".$v['branch_name'] ?>
        </a>
    <?php  }} ?>
<?php echo $pagebar;?>
</body>
