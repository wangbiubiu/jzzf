        ...
        <div class="navbar-header">
            ...
            <ul class="clearfix ">
                ...
                <dl <?php if (ROUTE_CONTROL != 'pay') echo 'class="nav_selected"'; ?>>
                    <?php if (!($this->eid > 0)) { ?><a href="/merchants.php?m=User&c=index&a=sindex"><?php } else { ?>
                ...
                <dl <?php if (ROUTE_CONTROL == 'pay') echo 'class="nav_selected"'; ?>>
                    <?php if (!($this->eid > 0)) { ?>
                        <?php if ($this->merchant['mtype'] == '1') { ?>
                            <a href="/merchants.php?m=User&c=pay&a=payconfig">
                            <?php } else { ?>     
                                <a href="/merchants.php?m=User&c=pay&a=ModifyPwd"> 
                                <?php } ?>        
                            <?php } else { ?>
                                <?php if ($_SESSION['E_LEVEL'] == 1) { ?>
                                    <a href="/merchants.php?m=User&c=modify&a=setPwd">
                                    <?php } else { ?>
                                        <a href="#">                  
                                        <?php }
                                    } ?>
                                            ...
                                            <?php if (!empty($this->employer) && isset($this->employer['account'])) { ?>
                                                ...
                                                <div class="downewm" style="text-align: center;line-height: 25px;"><span>用户名：<?php echo $this->merchant['username']; ?></span><br/>
                                                    <?php if (in_array($this->merchant['source'], array(1, 2, 3)) && !($this->merchant['mfypwd'] > 0)) { ?>
                                                        <span>默认密码：<?php $password = md5($this->merchant['thirduserid'] . '@me');
                                                        $password = substr($password, 2, 8) . 'cf' . $this->merchant['source'];
                                                        echo $password; ?></span>
    <?php } ?>
                                                </div>
<?php } ?>
