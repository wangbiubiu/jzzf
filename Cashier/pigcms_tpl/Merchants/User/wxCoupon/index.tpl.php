...
<head>
	...
	<style>
		...
		.setting_rows .i-checks label{cursor: pointer;}
            	...
            	<div class="col-lg-12">
            	    <div class="ibox float-e-margins">
							<!--div class="ibox-title clearfix">	
								...
								<?php 
									$str = "温馨提示：您现在的状态是";
									switch ($this->merchant['apply']){
										case 0:
											$str = '温馨提示：如果您没有公众号或您的公众号没有制券权限，您可以申请成为平台的子商户从而获得制券能力，我要<a href="/merchants.php?m=User&c=merchant&a=applyMerchant">申请</a>';
											break;
										case 1:
											$str .= "【请求申请成子商户】,暂时还不能制作券，请耐心等待审核";
											break;
										case 2:
											$str .= '【申请成子商户审核中】,暂时还不能制作券，请耐心等待审核，如果超过了三个工作日请去【申请制券】页查看一下状态<a href="/merchants.php?m=User&c=merchant&a=applyMerchant">查看状态</a>';
											break;
										case 3:
											$str .= "【申请成子商户审核通过】；您每月可制3张券，每张券的历史累计库存量不超过2000份。若上个月的领券量达到初级量级的80%以上，则制券张数不变，每张卡券的库存数提高至：累计不超过5000份。";
											break;
										case 4:
											$str .= '【申请成子商户审核被驳回】,暂时还不能制作券，请<a href="/merchants.php?m=User&c=merchant&a=applyMerchant">重新申请</a>';
											break;
										case 5:
											$str .= '【申请成子商户已过期】,暂时还不能制作券，请<a href="/merchants.php?m=User&c=merchant&a=applyMerchant">重新申请</a>';
											break;
         ...
         <table class="ui-table ui-table-list" data-page-size="20" style="padding: 0px;"> 
          ...
          <tbody class="js-list-body-region" id="table-list-body">
           ...
           <tr class="widget-list-item">
			...
			<td><?php echo $this->card_type[$ovv['card_type']]['zhname'];?></td> 
			...
			<button class="btn btn-sm btn-primary btn-primary-ewm" data-cdid="<?php echo $ovv['id'];?>" data-cardid="<?php echo $ovv['card_id'];?>" ><strong>查看二维码</strong></button> 
			
			<?php if(($ovv['card_type']==2 || $ovv['card_type']==4) && ($ovv['istowx']>0)){?>
			...
			<?php }else{?>
			<button class="btn btn-sm btn-gray"><strong>没有买单权限</strong></button>
			<?php }?>
			<a class="btn btn-sm btn-info"  href="?m=User&c=wxCoupon&a=cardetail&id=<?php echo $ovv['id'];?>" style="vertical-align:top;"> 详 情 </a>  <button onclick="deltheOrder(this,<?php echo $ovv['id'];?>);" class="btn btn-sm btn-danger"><strong> 删 除 </strong></button></td>
           </tr>
		   <?php }}else{?>
		   <tr class="widget-list-item"><td colspan="11">暂无卡券</td></tr>
		   <?php }?>
          </tbody> 
         </table> 
			...
			<div class="modal-content animated bounceInRight">
				...
				<div class="dialog_bd">
					...
					<div class="frm_control_group radio_row">
						...
						<div class="i-checks" id="istowxcard">
							<label>是否将卡券同步到微信：</label>
							<label for="toweixin">
							<input type="radio"  value="1" id="toweixin" name="iswxcard" checked="checked">&nbsp;同步&nbsp;
							<i></i>
							</label>
							<label for="notoweixin">
							<input type="radio"  value="0" id="notoweixin" name="iswxcard">&nbsp;不同步
							<i></i>
							</label>

							<p class="frm_tips">不同步卡券到微信将按照本站自己的领取核销逻辑处理</p>
						</div>
 ...
 $(document).ready(function(){
		$('.ui-table-list').footable();
		 ...
		 $("#Pop_Set_Cardtype .btn-confirm").click(function(){
			  ...
			  var iswxcard=$.trim($('#istowxcard input:checked').val());
			  typeid=parseInt(typeid);
			  iswxcard=parseInt(iswxcard);
			  if(typeid >= 0){
				 window.location.href="?m=User&c=wxCoupon&a=createKq&typeid="+typeid+'&iswx='+iswxcard;
			  }else{
			     alert('请选择一个卡券类型！');
				 return false;
			  }
		  ...
		  $(".btn-primary-ewm").click(function(){
			   if($('.modal-backdrop').size()>0 || getEwming || ($('.sweet-overlay').size()>0 && $('.sweet-overlay').attr('display')=='block')){
				...
				getEwming=true;
				if(cardid && cardid=='localCard_id'){
					$('#ewmPopDiv .modal-body').append('<img src="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=wxCoupon&a=qrcode&cdid=wapindex">');
				    $('body').append('<div class="modal-backdrop in"></div>');
					$('#ewmPopDiv .downewm').hide();
					$('#ewmPopDiv').show();	
					getEwming=false;
				}else{
