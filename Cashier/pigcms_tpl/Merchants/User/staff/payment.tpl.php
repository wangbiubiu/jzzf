...
<head>
	...
	.inmodal .modal-header{padding: 30px 15px 15px 15px;}
        .micropay{    width: 50%;
    margin: 0 25%;}
        .micropay>div>label{float: left; margin-right: 10px; width: 20%}
         .micropay>div>input{float: left; width: 70%;}
         .btn-primary {
...
.btn-primary{ margin-left:240px; margin-top: 20px}
ul li{list-style: none;}
.tu{ background: #ffffff;}
.tu>ul{ margin-bottom: 0px; width: 710px; margin: 0 auto;}
.tu>ul>li{float:left; margin: 30px 30px;}
	</style>
</head>
<body>
    <div id="wrapper">
		<?php include RL_PIGCMS_TPL_PATH.APP_NAME.'/'.ROUTE_MODEL.'/public/staffleft.tpl.php';?>
                	...
                	<div class="col-lg-12">
                	    <div class="tabs-container weixin">
                	        <ul class="nav nav-tabs">
                                    <li class="active" style="border-radius: 0px;width:100%; text-align: center; border-bottom: 1px solid #f3f3f3;"><a data-toggle="tab" href="#tab-1" style="border-radius: 0px; margin-right: 0px;"><?php  if($type==1){ echo '扫码收款';}else{echo '扫码退款';}?></a></li>
                	        </ul>
                               
                               <div class="tu"><ul class="clearfix"><li class="icon1"><img src="./Cashier/pigcms_static/image/icon1.png"></li><li class="icon2"><img src="./Cashier/pigcms_static/image/icon2.png"></li> <li class="icon3"><img src="./Cashier/pigcms_static/image/icon3.png"></li> <li><img src="./Cashier/pigcms_static/image/icon4.png"></li><li><img src="./Cashier/pigcms_static/image/icon5.png"></li></ul></div>
                	        ...
                	        <div class="tab-content">
                	            ...
                	            <div id="tab-2" class="tab-pane">
                	                <div class="panel-body">
                	                    <div class="row">
                                                <div class="col-sm-6 micropayRefund" style="margin-left:25%"></div>
                        				</div>
                	                </div>
                	            </div>
                	        </div>
	 ...
	 if(Ttype==2){
		...
		!function(a,b,wx){
			...
			c.tmpl = function(d){
				var e = {
					weixin : {
						//micropay : '<h3 class="m-t-none m-b">收款</h3><p>只适用微信扫码支付</p><p>扫码支付确认信息.</p><form role="form" action="" id="micropay"><div class="form-group"><label>收款备注</label> <input type="text" placeholder="收款备注" name="goods_name"  id="goods_name" class="form-control"></div><div class="form-group"><label>支付金额</label> <input type="text" placeholder="支付金额(至少0.01元)" name="goods_price" id="goods_price" class="form-control"></div><div><button class="btn btn-sm btn-primary pull-right m-t-n-xs" type="submit"><strong>扫码收款</strong></button></div></form>',
						micropay : '</p><form role="form" action="" id="micropay"><div class="form-group"><label>收款备注</label> <input type="text" placeholder="收款备注" name="goods_name"  id="goods_name" class="form-control"></div><div class="form-group"><label>支付金额</label> <input type="text" placeholder="支付金额(至少0.01元)" name="goods_price" id="goods_price" class="form-control"></div><div><button class="btn btn-sm btn-primary  m-t-n-xs" type="submit"><strong>扫码收款</strong></button></div></form>',
						micropayRefund : '<h3 class="m-t-none m-b">退款</h3><p>扫码支付退款</p><p>扫支付交易详情页的条形码来退款.</p><form role="form" action="?m=User&c=cashier&a=wxSmRefund" id="wxSmRefund"><div><button class="btn btn-sm btn-primary m-t-n-xs" type="submit"><strong>扫码退款</strong></button></div></form>',
					}
				}
				var f;
				$.each(d,function(g,h){
			...
			c.submit = function(d){
				//?m=User&c=cashier&a=pay
				//?m=User&c=alicashier&a=alipay
				var tmpStr='';
				...
				if(formType=='micropay'){
					//判断扫码类型       刷卡支付
					if(tmpStr.substr(0,1) == '1'){
						$('#micropay').attr("action","?m=User&c=cashier&a=pay")
					}else if(tmpStr.substr(0,1) == '2'){
						$('#micropay').attr("action","?m=User&c=alicashier&a=alipay")
					}
				}else if(formType=='wxSmRefund'){
					//刷卡退款
					if(tmpStr.substr(0,1) == '2'){
						$('#wxSmRefund').attr("action","?m=User&c=cashier&a=wxSmRefund")
					}else if(tmpStr.substr(0,1) == '1'){
						$('#wxSmRefund').attr("action","?m=User&c=alicashier&a=aliSmRefund")
					}
 				}
			...
			c.create = function(s){
				...
				if(is_weixin()){
					...
					if(f.find('form').find('.form-group').size()){
						f.find('form').find('.form-group').last().after('<div class="form-group"><label>刷卡授权码</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>请连接扫码枪扫码</span><input type="text" placeholder="刷卡支付授权码(请连接扫码枪扫码)" name="auth_code" class="form-control auth_code"></div>');
					}else{
         ...
         <script>
              $(".icon1").click(function(){
                ...
                $(this).children("img").attr("src","./Cashier/pigcms_static/image/icon1_1.png");
                $(".icon2").children("img").attr("src","./Cashier/pigcms_static/image/icon2.png");
                $(".icon3").children("img").attr("src","./Cashier/pigcms_static/image/icon3.png");
              });
              $(".icon2").click(function(){
                $(this).children("img").attr("src","./Cashier/pigcms_static/image/icon2_2.png");
                $(".icon1").children("img").attr("src","./Cashier/pigcms_static/image/icon1.png");
                $(".icon3").children("img").attr("src","./Cashier/pigcms_static/image/icon3.png");
              });
              $(".icon3").click(function(){
                $(this).children("img").attr("src","./Cashier/pigcms_static/image/icon3_3.png");
                $(".icon1").children("img").attr("src","./Cashier/pigcms_static/image/icon1.png");
                $(".icon2").children("img").attr("src","./Cashier/pigcms_static/image/icon2.png");
             });
        </script>
