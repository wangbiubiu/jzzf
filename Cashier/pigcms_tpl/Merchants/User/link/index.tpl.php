...
<?php foreach($data[0] as $key=>$m){?>
	...
	<?php }else{ ?>
		<li>
			...
			<div class="mright">
				<?php if($m['sub']){ ?>
					<a href="?m=User&c=link&a=detailed&module=<?php if (!$m['linkurl']){ echo $m['module']; }else{ echo $m['linkurl']['a'];?>&method=<?php echo $m['linkurl']['module']; } ?>&url=<?php echo $info['url']; ?>&token=<?php echo $info['token']; ?>">详细</a>
				<?php } ?>
