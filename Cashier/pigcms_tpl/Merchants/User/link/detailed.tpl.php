...
<link href="<?php echo PIGCMS_TPL_STATIC_PATH;?>link/css/style_2_common.css?BPm" rel="stylesheet" type="text/css" />
<link href="<?php echo PIGCMS_TPL_STATIC_PATH;?>link/css/style.css" rel="stylesheet" type="text/css" />
<style>
...
<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;">使用方法：点击对应内容后面的“选中”即可。<a href="?m=User&c=link&a=index&url=<?php echo urlencode($url);?>&token=<?php echo $data['token'];?>">点击这里返回模块选择</a></div>
<?php echo $content; ?>
<script>
	function returnHomepage(url){
		$('.externalLink', parent.document).val(url).keyup();
