    ...
    <?php include RL_PIGCMS_TPL_PATH.APP_NAME.'/'.ROUTE_MODEL.'/public/leftmenu.tpl.php';?>
        ...
        <div class="row wrapper page-heading iconList">
                ...
                <div class="wrapper wrapper-content animated fadeInRight" style="width: 100%; background: #FFFFFF;">
                    <div class="row">
                    <form action="?m=Agent&c=statistics&a=forSalesman" method="get">
                    <input type="hidden" name='m' value="Agent">
                    <input type="hidden" name='c' value="statistics">
                    <input type="hidden" name='a' value="forSalesman">
                        <div id="dataselect" class="form-group" style="padding: 0 10px;">
                            <div id="datepicker" class="input-daterange">
                             <label class="font-noraml">业务员名称</label>
                                ...
                                <input class="input form-control" type="text" name='username' placeholder="输入业务员名称" style="width: 20%;border-radius: 2px;height: 30px;" value="<?php if(isset($getdata['username'])) echo $getdata['username'];?>">

                                <label class="font-noraml">选择日期</label>&nbsp;&nbsp;&nbsp;
                                <input type="text" value="<?php if(isset($getdata['start'])){echo $getdata['start'];}else{echo date('Y-m-d');}?>" name="start" class="input-sm form-control" id="datestart" placeholder="开始时间" style=" margin-bottom: 0px; width: 15%; height: 30px; border-radius:3px">
                                &nbsp;<span> 到 </span>&nbsp;
                                <input type="text" value="<?php if(isset($getdata['end'])){echo $getdata['end'];}else{echo date('Y-m-d');}?>" name="end" class="input-sm form-control" id="dateend" placeholder="结束时间" style=" margin-bottom: 0px; width: 15%;height: 30px;border-radius:3px">
                                &nbsp;&nbsp;&nbsp;<input class="btn btn-primary" type="submit" value="查 询" style="width:70px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-primary"  style="width:100px;" href="?m=Agent&c=statistics&a=data2ExcelSalesman" >导出到excel</a>
                            </div>
                        </div>
                </form>
                        <table class="store" border="1" bordercolor="#f2f2f2" width="98%" style="margin-left:1%;">
                            ...
                            <?php  foreach ($salers as $vk => $ovv){ ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p><?php if (isset($ovv['static']['weixin']['num'])) {echo round($ovv['static']['weixin']['num'],2);}else{echo 0;} ?></p>

                                        <p><?php if (isset($ovv['static']['alipay']['num'])) {echo round($ovv['static']['alipay']['num'],2);}else{echo 0;} ?></p>
                                    </td>
                                    <td>
                                    ...
                                    <td><a style='width: 100%;height: 100%;' href="?m=Agent&c=statistics&a=salerInfo&id=<?php echo $ovv['id']; ?><?php  if (!empty($getdata['start'])){ echo '&start='.$getdata['start']; } ?><?php  if (!empty($getdata['end'])){ echo '&end='.$getdata['end']; } ?>"><button style='padding: 0px;'>查看流水</button></a></td>
                                </tr>
                        ...
                        <div class="hz clearfix">
                            ...
                            <p>总收入：<?php echo round($income,2); ?>元</p>
                            <p>总笔数：<?php echo $num; ?>笔</p>
                            <p>总金额：<?php echo round($sum,2); ?>元</p>
