...
<head>
	...
	<style>
		...
		.ui-centered-image img,.isproxyimage img{cursor: pointer;}
		.ui-centered-image,.isproxyimage { position: relative;}
            ...
            <div class="row">
            	<div class="col-lg-12">
            	    <div class="ibox float-e-margins">
						...
						<li> <a href="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=cashier&a=payRecord">微信收款</a> </li> 
         ...
         <table class="ui-table ui-table-list" data-page-size="20" style="padding: 0px;"> 
          ...
          <tbody class="js-list-body-region">
           ...
           <tr class="widget-list-item">
			...
			<?php if(!empty($ovv['ewmurl']) && ($ovv['ewmurl']=='isproxy')){?>
			<td><div class="isproxyimage">
			 <img src="<?php echo $this->RlStaticResource;?>images/qrcode.png" width="90">
			 <input type="hidden" value="<?php echo $ovv['id']?>">
			</div></td> 
			<?php }else{?>
		   ...
		   <?php }}else{?>
		   <tr class="widget-list-item"><td colspan="10">暂无记录</td></tr>
		   <?php }?>
          </tbody> 
         </table> 
    ...
    <script>
	   ...
	   if(bodyW>350) bodyW=350;
        $(document).ready(function(){
		   ...
		   $(".isproxyimage").click(function(){
		     	var oid=$(this).find('input').val();
				$.post('?m=User&c=cashier&a=proxySMewm',{oid:oid},function(ret){
				    if(ret.ewmurl){
						$('#ewmPopDiv .modal-body').qrcode({ 
							//render: "table", //table方式 
							width: bodyW, //宽度 
							height:bodyW, //高度 
							text:ret.ewmurl//任意内容 
					   });
					   $('body').append('<div class="modal-backdrop in"></div>');
					   $('#ewmPopDiv').show();
					}else{
					    swal("错误提示", ret.msg , "error");
					}
				},'JSON');
