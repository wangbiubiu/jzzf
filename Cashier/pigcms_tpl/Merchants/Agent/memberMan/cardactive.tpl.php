			...
			<div class="row wrapper border-bottom white-bg page-heading">
                <div class="col-lg-10">
                    ...
                    <ol class="breadcrumb">
                        <li><a>User</a></li>
                        <li><a>memberMan</a></li>
                        <li class="active"><strong>cardactive</strong></li>
                    </ol>
                </div>
							...
							<div class="ibox-content">
								...
								<div class="appmsg_edit_item radio_item"> 
									<div class="frm_control_group radio_row"> 
										...
										<div class="frm_controls frm_vertical_lh"> 
											<div class="radio_control_group group" id="required_form"> 
												<?php foreach ($key_val as $key => $val) {
												  if($key=='USER_FORM_INFO_FLAG_INCOME' || $key=='USER_FORM_INFO_FLAG_HABIT'){
												     continue;
												  }
												?>
												<label class="frm_radio_label frm_radio_input" for="<?php echo $key;?>"> 
								...
								<div class="appmsg_edit_item radio_item"> 
									<div class="frm_control_group radio_row"> 
										...
										<div class="frm_controls frm_vertical_lh"> 
											<div class="radio_control_group group" id="optional_form"> 
												<?php foreach ($key_val as $key => $val) {
												   if($key=='USER_FORM_INFO_FLAG_INCOME' || $key=='USER_FORM_INFO_FLAG_HABIT'){
												     continue;
												  }
												?>
												<label class="frm_radio_label frm_radio_input" for="<?php echo $key;?>1"> 
												<i class="icon_checkbox"></i>
...
$(document).ready(function() {
	  $('#sub_add_card').click(function(){
	  		$.ajax({
	  			...
	  			success:function(ret){
	  				if(!ret.error){
	  				   window.location.href="?m=User&c=memberMan&a=cardindex";
	  				}else{
	  					swal({
	  					  title: "保存失败！",
	  					  text: ret.msg,
	...
	$('#required_form input').click(function(){
	    var idds=$(this).attr('id');
		if($(this).prop("checked")){
			$('#'+idds+'1').prop("checked", false);
		}
	});

 	$('#optional_form input').click(function(){
	    var idds=$(this).attr('id');
		idds=idds.replace('1','');
		if($(this).prop("checked")){
		 $('#'+idds).prop("checked", false);
		}
	});
