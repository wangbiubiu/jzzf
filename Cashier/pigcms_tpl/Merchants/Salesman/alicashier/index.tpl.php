...
<head>
	...
	<style>
		...
		#qr-code-forever {line-height: 20px;padding-top: 6px;}
		#qr-code-forever p{margin-top: 80px;}
         ...
         <div class="pay-config" style="margin-top: 7%;"> 
		  ...
		  <div class="control-group config-title"> 
           ...
           <div class="control-group config-amount"> 
            <div class="controls"> 
             <input type="text" name="cashier_value" class="js-cashier-value js-input" value="" placeholder="输入金额(至少0.01元)" /> 
	 ...
	 var thismoney=0;
        $(document).ready(function(){
			...
			$("#qr-code-autopay").html('').css('background-color','#FFF').qrcode({ 
					...
					width: qwidth, //宽度 
					height: qheight, //高度
					text:'<?php echo $this->SiteUrl;?>/merchants.php?m=Index&c=pay&a=aliautopay&mid=<?php echo $this->mid;?>&eid=<?php echo $this->eid;?>&storeid=<?php echo $this->storeid;?>' //任意内容 
				});
			$('.js-create-qrcode').click(function(){
			     if(!topost) return false;
				 var postdata={paytype:'alipay'};
				 ...
				 topost=false;
				 $.post('?m=User&c=alicashier&a=getAliEwm',postdata,function(ret){
					topost=true;
					if(!ret.error){
						...
						$("#qr-code-forever").html('').css('background-color','#FFF').qrcode({ 
							...
							width: qwidth, //宽度 
							height: qheight, //高度 
							text:"<?php echo $this->SiteUrl;?>/merchants.php?m=Index&c=pay&a=aliforeverpay&ordid="+ret.ewminfo //任意内容 
						});
			...
			$('.js-refresh-list').click(function(){
				if(is_mobile()){
			     ...
			     $.ajax({
					url: "?m=User&c=alicashier&a=getajaxOrder&cf=index",
					type: "POST",
					dataType: "json",
					/*async:true,
					data:{cf:'index'},*/
					success: function(res){
						if(!res.error && res.datas){
							...
							$.each(res.datas,function(kk,vv){
							   ...
							   datahtml+='<td>'+vv.storename+'</td>';
							   datahtml+='<td>'+vv.optername+'</td>';
							   datahtml+='<td>';
							   if(vv.comefrom > 0){
							     datahtml+='<button class="btn btn-sm btn-success btn-st"><strong> 已支付 </strong></button> ';
							   }else if(vv.refund!=2 && vv.refund!=1){
							      datahtml+='<button class="btn btn-sm btn-warning" onclick="aliRefundBtn(this,'+vv.id+','+vv.mid+');"><strong> 退&nbsp;&nbsp;&nbsp;款 </strong></button> ';
							   }else{
