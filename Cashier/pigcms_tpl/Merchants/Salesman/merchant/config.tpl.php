...
.weixin>div{ min-height: 60px; border-top:1px solid #f2f2f2;}
.weixin>div>label{width: 150px; text-align: right; margin-right: 10px; display: block;float: left;}
.weixin>div>input{float: left; margin-top: 18px; margin-right: 10px;}
				...
				<div class="wrapper wrapper-content animated fadeInRight">
					...
					<div class="row payment_allocation">
						<div class="configure">
							...
							<div class="weixin">
								<h2>微信支付配置<span>（收款通道暂未通过审核，审核通过方可保存）</span><input type='button' id='weixinpay' value="保存" style="float: right; height: 30px; background: none; color:green; line-height: 30px; border:none;"></h2>
								<!-- <div class="clearfix">
									<label>是否认领商户：</label><input type="checkbox" />注：该商户为认领商户，由所属通方提供分润
								</div> -->
								<div class="clearfix">
									<label style="margin-top: 18px">微支付商户号</label>
									<input name='weixin[mchid]' class="shh" type="text" placeholder="请输入商户号" value='<?php echo $payConfig['configData']['weixin']['mchid'];?>' /><span class="ts" style="color: red;display: none;">请填写商户号</span>
								</div>


								<div style="line-height: 25px">
									<label></label><input type="checkbox" id="gaoji"  value="on" style="margin-top: 6px"/>高级选项
									<div class="gaoji_nr" style="display: none;">
										
										<p>
											<label>appID</label>
											<input class="appid" type="text" value='<?php echo $payConfig['configData']['weixin']["appid"];?>' name='weixin[appid]' placeholder="请输入微信公众号" />
											<i class="wh fa fa-question-circle"></i>
											<i class="du fa  fa-check-circle"></i>
										</p>
										<p>
											<label>AppSecret</label>
											<input class="appSecret" type="text" name='weixin[appSecret]' value='<?php echo $payConfig['configData']['weixin']["appSecret"];?>' placeholder="请输入微信公众号密匙" />
											<i class="wh fa fa-question-circle"></i>
											<i class="du fa  fa-check-circle"></i>
										</p>
										<p>
											<label>API密钥</label>
											<input class="partnerKey" type="text" name="weixin[partnerKey]"  value='<?php echo $payConfig['configData']['weixin']["partnerKey"];?>' placeholder="请输入微信支付商户号初始密匙" />
											<i class="wh fa fa-question-circle"></i>
											<i class="du fa  fa-check-circle"></i>
										</p>
							...
							<div class="zfb clearfix">
								<h2>支付宝2.0收款参数配置<span>（收款通道暂未通过审核，审核通过方可保存）</span><input type='button' value="保存"  id='alipay' style="float: right; height: 30px; background: none; color:green;line-height: 30px; border:none;"></h2>
								<!-- 	<div>
								...
								<!-- 	<div>
									...
									<div style="line-height: 25px; padding-top:10px;">
										<label >appID:</label><input class="zfbsh" type="text" name='alipay[appID]' placeholder="请输入支付宝商户号" value="<?php echo $payConfig['configData']['alipay']['appID']; ?>" />
									</div>
							</div>
...
$('#weixinpay').click(function(){
	...
	$.post('?m=Salesman&c=merchant&a=config',{data:htmlToArray(payConfigData)},function(result){
		if(result.status == 1){
			swal({
				title: "成功",
				text: result.msg,
...
$('#alipay').click(function(){
	...
	$(".dropz").dropzone({
		url: "?m=Salesman&c=merchant&a=pem_upload",
