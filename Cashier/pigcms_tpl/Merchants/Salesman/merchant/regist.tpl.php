...
<?php include RL_PIGCMS_TPL_PATH.APP_NAME.'/'.ROUTE_MODEL.'/public/top.tpl.php';?>
      <div class="row wrapper border-bottom white-bg page-heading">
		...
		<form name='regist' action=""  method="post" class="wrapper wrapper-content animated fadeInRight">
					...
					<div class="clearfix" id="comDescription" style="display: none;" >
						...
						<div style="float:left ;">
							<div id="merchant_description" style=" width: 600px; height: 110px; border: 1px solid #f2f2f2;" contenteditable>
								<textarea name='dealdesc' style='width: 100%;height: 100%;'></textarea>
							</div>
							<p style="color: red;"></p>
...
$(".js_pic_url,.js_pic_url .icon20_common add_gray").dropzone({
	url: "?m=Salesman&c=merchant&a=upcert",
...
$.post('?m=Salesman&c=merchant&a=getSecondLevel',{id:provinceid},function(ret){
if(ret.data){
...
$.post('?m=Salesman&c=merchant&a=getThirdLevel',{id:cityid},function(ret){
if(ret.data){
  $.each(ret.data,function(nn,vv){
    console.log(vv);
   cityHtml+='<option value="'+vv.id+'" data-fullname="'+vv.name+'"  >'+vv.name+'</option>';
...
function GetCircle(){
  ...
  $.ajax({
  	type:'post',
  	url:'?m=Salesman&c=merchant&a=getFourthlevel',
  	dataType:'json',
  	data:{id:obj},
  	success:function (ev) {
