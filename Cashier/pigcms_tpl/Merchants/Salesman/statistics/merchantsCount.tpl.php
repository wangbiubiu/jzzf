						...
						<div class="ibox float-e-margins">
							<div class="ibox-title">
								<div class="form-group" style="border-bottom: 1px solid #cccccc; font-size: 18px;">
									<label class="font-noraml"><?php echo $cashier_merchants[0]['company'].'的流水统计';?></label>
								...
								<!--
                                	...
                                	描述：选择日期
                                -->
                      <form action="merchants.php?m=Salesman&c=statistics&a=merchantsCount" method="get">        
    						<div id="dataselect" class="form-group" style="padding: 0 10px;">
    						     ...
    						     <input type="hidden" name="a" value="merchantsCount">
                                	...
                                	<table border="1" class="payment" style="margin: 30px 30px 30px 0px;" width="60%" bordercolor="#e0e0e0">
                                		<tbody>
                                			...
                                			<tr>
                                				...
                                				<td><?php if($start && $end){echo date('Y-m-d',$start) .'&nbsp至&nbsp'. date('Y-m-d',$end);}else{echo 全部 ;} ?></td>
                                				<td><?php if($order_count3[0]['goods_price']){echo $order_count3[0]['goods_price'];}else{echo '0.00';}?></td>
                                				<td><?php if($order_count3[0]['income']){echo $order_count3[0]['income'];}else{echo '0.00';}?></td>
                                			</tr>
                                			<?php if(!$_GET['pay_way']){?>
                                    			 <tr>
                                    				<td>微信支付</td>
                                    				<td><?php if($order_count1[0]['goods_price']){echo $order_count1[0]['goods_price'];}else{echo '0.00';}?></td>
                                    				<td><?php if($order_count1[0]['income']){echo $order_count1[0]['income'];}else{echo '0.00';}?></td>
                                    			</tr>
                                        		<tr>
                                    				<td>支付宝城市</td>
                                    				<td><?php if($order_count2[0]['goods_price']){echo $order_count2[0]['goods_price'];}else{echo '0.00';}?></td>
                                    				<td><?php if($order_count2[0]['income']){echo $order_count2[0]['income'];}else{echo '0.00';}?></td>
                                    			</tr>
                                			<?php }?>
                                			<?php if($_GET['pay_way'] == 'weixin'){?>
                                    			<tr>
                                    				<td>微信支付</td>
                                    				<td><?php if($order_count1[0]['goods_price']){echo $order_count1[0]['goods_price'];}else{echo '0.00';}?></td>
                                    				<td><?php if($order_count1[0]['income']){echo $order_count1[0]['income'];}else{echo '0.00';}?></td>
                                	...
                                	<table border="1" class="payment1" style="margin: 30px 30px 30px 0px;" width="100%" bordercolor="#e0e0e0">
                                		...
                                		<?php foreach ($order_list as $key => $val){?>
                                			<tr>
                                				...
                                				<td><?php if($val['refund'] == 2){echo $val['goods_price'];}else{echo 0;}?></td>
                                				<td><?php if($val['refund'] == 2){echo 0;}else{echo $val['income'];}?></td>
                                				<td><?php echo date('Y-m-d H:i:s',$val['paytime'])?></td>
                                				<td><?php if($val['pay_way'] == 'weixin'){echo '微信支付';}else if($val['pay_way'] == 'alipay'){echo "支付宝城市";}?></td>
                                				<td><?php echo $val['goods_describe'];?></td>
                                				<td><?php echo $val['business_name']?></td>
                                				<td><?php if($val['refund'] == 2){echo "<p class='ytk'>已退款</p>"; }?></td>
                                			</tr>
                                		<?php }?>
                                		</tbody>
                                	</table>
...
<form action="/merchants.php?m=Salesman&c=statistics&a=MerchantsOrderExcel" method="post" id="MerchantsOrderExcel">
    <input type="hidden" name="mid" value="<?php echo $mid; ?>">
</form>
...
$("#side-menu li").click(function () {
   ...
   $(this).find('.nav-second-level').css('display','block').css('min-width','140px');
    ...
    var doughnutData_m = [
        {
            value: <?php if($order_count1[0]['goods_price']){echo $order_count1[0]['goods_price'];}else{echo 0;}?>,
            color: "#33EA90",
            highlight: "#4BFFA8",
            label: "微信支付"
        },
        {
            value: <?php if($order_count2[0]['goods_price']){echo $order_count2[0]['goods_price'];}else{echo 0;}?>,
            color: "#00a3d2",
            highlight: "#24c7f6",
            label: "支付宝城市"
        },
