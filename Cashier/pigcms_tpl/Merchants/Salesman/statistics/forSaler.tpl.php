...
<body>
	...
	<?php include RL_PIGCMS_TPL_PATH.APP_NAME.'/'.ROUTE_MODEL.'/public/leftmenu.tpl.php';?>
        ...
        <div class="row wrapper page-heading iconList">
                ...
                <div class="wrapper wrapper-content animated fadeInRight" style="width: 100%; background: #FFFFFF;">
                	<div class="row">
                	<form action="/merchants.php?m=Salesman&c=statistics&a=forSaler" method="get">
	                	<div id="dataselect" class="form-group" style="padding: 0 10px;">
							...
							<div id="datepicker" class="input-daterange">
                				...
                				<input class="form-control" type="text" placeholder="商户名" style="width: 17%;border-radius: 7px;height: 30px; margin-bottom: 0px;" name='username' value="<?php if(isset($getdata['username'])) echo $getdata['username'];?>">
								<label class="font-noraml">选择日期</label>&nbsp;&nbsp;&nbsp;
								<input type="text" value="<?php if(isset($getdata['start'])){echo $getdata['start'];}else{echo date('Y-m-d');}?>" name="start" class="input-sm form-control" id="datestart" placeholder="开始时间" style=" margin-bottom: 0px; width: 20%; height: 30px;" name='start'>
								&nbsp;<span> 到 </span>&nbsp; 
								<input type="text" value="<?php if(isset($getdata['end'])){echo $getdata['end'];}else{echo date('Y-m-d');}?>" name="end" class="input-sm form-control" id="dateend" placeholder="结束时间" style=" margin-bottom: 0px; width: 20%;height: 30px;" name='end'>
	                	...
	                	<table class="store" border="1" bordercolor="#f2f2f2" width="98%" style="margin-left:1%;">
	                		<tbody>
	                			...
	                			<tr>
	                				...
	                				<td>
	                					<p><?php if(!empty($v['weixin'])){echo round($v['weixin']['total_goods_price'],2);}else{echo 0.00;}?>&nbsp元</p>
	                					<p><?php if(!empty($v['alipay'])){echo round($v['alipay']['total_goods_price'],2);}else{echo 0.00;}?>&nbsp元</p>
	                				</td>
	                				<td>
	                					<p><?php if(!empty($v['weixin'])){echo $v['weixin']['total_num'];}else{echo 0;}?></p>
	                					<p><?php if(!empty($v['alipay'])){echo $v['alipay']['total_num'];}else{echo 0;}?></p>
	                				</td>
	                				
	                				<td>
	                					<p><?php if(!empty($v['commission'])){echo $v['commission'] * 100 .'%';}else{echo 0;}?></p>
	                					<p><?php if(!empty($v['alicommission'])){echo $v['alicommission'] * 100 .'%';}else{echo 0;}?></p>
	                				</td>
	                				
	                				
	                				<td>
	                					<p><?php if(!empty($v['weixin'])){echo round($v['weixin']['total_income'],2);}else{echo 0.00;}?>&nbsp元</p>
	                					<p><?php if(!empty($v['alipay'])){echo round($v['alipay']['total_income'],2);}else{echo 0.00;}?>&nbsp元</p>
	                				</td>

	                				<td><a href="/merchants.php?m=Salesman&c=statistics&a=merchantsCount&mid=<?php echo $v['mid'];?>"><button>查看</button></a></td>
	                			</tr>
	                	...
	                	<div class="hz clearfix">
	                		<p>我的佣金总收入：<?php if($order_count[0]['income']){echo round($order_count[0]['income'],2);}else{echo '0.00';}?>元</p>
	                		<p>总笔数：<?php echo $order_count[0]['count']?>笔</p>
	                		<p>总金额：<?php if($order_count[0]['goods_price']){echo round($order_count[0]['goods_price'],2);}else{echo '0.00';}?>元</p>
	                	</div>
   ...
   <div class="modal inmodal" tabindex="-1" role="dialog"  id="Export_excel_pop">
	...
	<form action="/merchants.php?m=Salesman&c=statistics&a=MerchantsCountExcel" method="post" id="MerchantsCountExcel">
