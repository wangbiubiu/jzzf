                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content yc" style="border-top:none">
                            <form action="/merchants.php?m=System&c=settlement&a=abank" method="get">
                                <input type="hidden" value="System" name="m" >
                                <input type="hidden" value="settlement" name="c" >
                                <input type="hidden" value="abank" name="a" >
                                <div id="dataselect" class="form-group" style="padding: 0 10px;">
                                    <div id="datepicker" class="input-daterange">
                                        <label class="font-noraml">代理商ID</label>
                                        <input class="input form-control" type="text"  name="aid" {pg: if (isset($data.aid))}{pg: $data.aid}{pg:/if} placeholder="输入代理商编号" style="width: 17%;border-radius:3px;height: 30px; margin-bottom: 0px;">
                            ...
                            <div class="employersDelAll" >
                                <table class="footable table table-stripped toggle-arrow-tiny" data-page-size="10" data-filter=#filter style="margin-bottom: 0px;">
                                    ...
                                    <tr>
                                        ...
                                        <th style="text-align: center;"  data-hide="phone">持卡人姓名</th>
                                    ...
                                    {pg:foreach item=vv from=$bank}
                                    <tr class="widget-list-item bd_nr" style="text-align: center;height:50px;">
                                        <td>{pg:$vv.uname}</td>
                                        <td>{pg: if ($vv.isCompay==0)}个人{pg: else}公司{pg: /if}</td>
                                        <td>{pg:$vv.customerName}</td>
                                        <td>
                                            {pg: if ($vv.cerdType=="01")}身份证{pg:/if}
                                            {pg: if ($vv.cerdType=="02")}军官证{pg:/if}
                                            {pg: if ($vv.cerdType=="03")}护照{pg:/if}
                                            {pg: if ($vv.cerdType=="04")}回乡证{pg:/if}
                                            {pg: if ($vv.cerdType=="05")}台胞证{pg:/if}
                                            {pg: if ($vv.cerdType=="06")}警官证{pg:/if}
                                            {pg: if ($vv.cerdType=="07")}士兵证{pg:/if}
                                            {pg: if ($vv.cerdType=="99")}其它证件{pg:/if}
                                        </td>
                                        <td>{pg:$vv.cerdId}</td>
                                        <td>
                                        ...
                                        <td>{pg:$vv.phoneNo}</td>
                                        <td>
                                            {pg: if ($vv.bank==0)}未审核{pg:/if}
                                            {pg: if ($vv.bank==1)}审核通过{pg:/if}
                                            {pg: if ($vv.bank==2)}审核失败{pg:/if}
                                        </td>
                                        <td>{pg:$vv.bankmsg}</td>
                                        <td>
                                            <button class="btn btn-sm btn-info" onclick="window.open('/merchants.php?m=System&c=settlement&a=card&id={pg:$vv.id}')" style="background: #44b549;">查看证件</button>
                                            {pg: if ($vv.bank==0)}
                                            <button class="btn btn-sm btn-info setmbank" data-id="{pg:$vv.id}" style="background: #008fd3;">通过</button>
                                            <button class="btn btn-sm btn-danger" data-id="{pg:$vv.id}" onclick="$('.bankmsg').fadeIn();$('.bankid').val('{pg:$vv.id}')" style="background: #ed5565;">拒绝</button>
                                            {pg: /if}
                                        </td>
                                    </tr>
                                    {pg:/foreach}
                                ...
                                <p style="text-align: right; padding-right: 150px; height: 50px; background: #f2f2f2; line-height: 50px;">合计数据：{pg:$total} 条</p>
                            </div>
                            {pg:$page}
                        </div>
    ...
    <div style="width: 500px;height: 300px;background-color: #fff;border-radius: 5px;margin: auto;top: 0;left: 0;right: 0;bottom: 0;position: fixed;overflow: hidden;">
        ...
        <div style="width: 440px;margin: 50px auto 0;text-align: center;">
            ...
            <div style="margin-top: 30px;">
                <button class="btn btn-sm btn-info setmbank2" data-id="{pg:$vv.id}" style="background: #008fd3;height: 40px;width: 200px;">失败提交</button>
            </div>
        </div>
    </div>
</div>
