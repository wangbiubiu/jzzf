    ...
    <div id="page-wrapper" class="gray-bg">
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="title clearfix">
                <ul>
                    <li {pg: if ($type==1)}class='cent'{pg:/if} >微信支付配置</li>
                    <li {pg: if ($type==2)}class='cent'{pg:/if} >支付宝支付配置</li>
                </ul>
            </div>
                ...
                <div class="configure">
                    <form id="wxform" action="" method="post" enctype="multipart/form-data" data-mode='weixin'>
                        <div class="weixin">         <h2>微信支付配置
                            ...
                            <div class="clearfix">
                                <input type="hidden" name="weixin[mid]" value="{pg:$mid}">
                                <div><label>mchID：&nbsp;&nbsp;</label><input style="line-height: 24px" class="shh" type="text" name="weixin[mchid]" value="{pg:$payConfig.configData.weixin.mchid}" placeholder="请输入商户号" onkeyup="this.value=this.value.replace(/\D/g,'')"/><span class="ts" style="color: red;display: none;">请填写商户号</span></div>
                                <div><label>商户名称：</label><input style="line-height: 24px" class="mc" type="text" name="weixin[nickname]" value="{pg:$wxname}" placeholder="请输入商户名称" /><span class="ts" style="color: red;display: none;">请填写商户名称</span></div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="gaoji_nr" style="display: none;">
                                    ...
                                    <p>
                                        <label>appID</label>
                                        <input class="appid" type="text" name='weixin[appid]' style="line-height: 24px" value="{pg:$payConfig.configData.weixin.appid}" placeholder="请输入微信公众号" onkeyup="value=value.replace(/[^\a-\z\A-\Z0-9\@\.]/g,'')"/>
                                        <i class="wh fa fa-question-circle"></i>
                                        <i class="du fa  fa-check-circle"></i>
                                    </p>
                                    <p>
                                        <label>appSecret</label>
                                        <input class="appSecret" type="text"  style="line-height: 24px" name='weixin[appSecret]' value="{pg:$payConfig.configData.weixin.appSecret}" placeholder="请输入微信公众号密匙" onkeyup="value=value.replace(/[^\a-\z\A-\Z0-9\@\.]/g,'')"/>
                                        <i class="wh fa fa-question-circle"></i>
                                        <i class="du fa  fa-check-circle"></i>
                                    </p>
                                    <p>
                                        <label>partnerKey</label>
                                        <input class="partnerKey" type="text"  style="line-height: 24px" name="weixin[key]" value="{pg:$payConfig.configData.weixin.key}" placeholder="请输入微信支付商户号初始密匙" onkeyup="value=value.replace(/[^\a-\z\A-\Z0-9\@\.]/g,'')"/>
                                        <i class="wh fa fa-question-circle"></i>
                                        <i class="du fa  fa-check-circle"></i>
                                    </p>
                                    <p>
                                        <label>apiclient-cert.pem</label>
                                        <input class="apiclient_cert form-control" {pg: if $payConfig.configData.weixin.apiclient_cert } value="pem文件已上传" readonly="readonly" {pg:/if} type="text" placeholder="请输入微信支付商户证书"/>
                                        <input type="hidden" name="weixin[apiclient_cert]" placeholder="apiclient_cert私钥文件" value="{pg:$payConfig.configData.weixin.apiclient_cert}"  class="hiddeninput">
                                        <a class="dropz">浏览</a>
                                        <i class="wh fa fa-question-circle"></i>
                                        <i class="du fa  fa-check-circle"></i>
                                    </p>
                                    <p>
                                        ...
                                        <input class="apiclient_key form-control"  {pg: if $payConfig.configData.weixin.apiclient_key} value="pem文件已上传" readonly="readonly" {pg:/if} type="text" placeholder="请输入微信支付商户证书密匙安全"/>
                                        <input type="hidden" name='weixin[apiclient_key]' placeholder="微信支付rootca文件" value="{pg:$payConfig.configData.weixin.apiclient_key}"  class="hiddeninput">
                                        <a class="dropz">浏览</a>
                                        <i class="wh fa fa-question-circle"></i>
                                        <i class="du fa  fa-check-circle"></i>
                                    </p>
                    ...
                    <form style="display: none;" action="" name='ali' method="post" data-mode='alipay'>
                        <div class="zfb clearfix">
                            <h2>支付宝2.0收款参数配置
                                ...
                                <span>（收款通道暂未通过审核，审核通过方可保存）</span>
                                {pg: else}
                                   <span style="color: green">（审核通过） <input type="button" class="btn-confirm" value="保存" name="" style="float: right; height: 30px; background: none; color:green;line-height: 30px; border:none;">
                                    {pg:/if}
                            </h2>
                            ...
                            <div>
                                {pg: if $mtype == '1'}
                                <label>授权token:</label><input style="line-height: 24px;width: 450px;" class="zfbtoken" type="text" value="{pg:$payConfig.configData.alipay.appauthtoken}" name='alipay[appauthtoken]' placeholder="请输入商户授权token" />
                                {pg: else}
                                <label>appID:</label><input style="line-height: 24px;" class="zfbsh" type="text" value="{pg:$payConfig.configData.alipay.appID}" name='alipay[appID]' placeholder="请输入支付宝商户号" onkeyup="this.value=this.value.replace(/\D/g,'')"/>
                                {pg:/if}
                            </div>
                            <div><label>商户名称：</label><input style="line-height: 24px" class="zfbmc" type="text" name="alipay[appNickname]" value="{pg:$aliname}" placeholder="请输入商户名称" /><span class="ts" style="color: red;display: none;">请填写商户名称</span></div>
                            <input type="hidden" name="alipay[mid]" value="{pg:$mid}">
                        </div>
        ...
        $('.btn-confirm').click(function(){
            if($(this).parents('form').attr('data-mode')=='weixin'){
                if($("#gaoji").is(":checked")){
                    ...
                    if($(".shh").val().length==10 || $(".shh").val().length==12){

                    }
                    else{
                        swal("商户号长度为10位或12位","", "error");
                        return false;
                    }
                }
            }else{
