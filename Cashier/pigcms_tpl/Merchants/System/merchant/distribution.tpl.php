...
<link href="{pg:$smarty.const.PIGCMS_TPL_STATIC_PATH}plugins/css/footable/footable.core.css" rel="stylesheet">
<style>
.ibox-title h5 {
	margin: 10px 0 0px;
}
			...
			<div class="wrapper wrapper-content animated fadeInRight">
				...
				<div class="content">
                                    <form action="/merchants.php?m=System&c=merchant&a=distribution" method="post">
                                        <input type="hidden" name="mid" value="{pg:$mer.mid}">
						<p><label>商户名:</label><span>{pg:$mer.company}</span></p>
						<p><label>分配:</label>
							<label>
								代理商：
                                                                <select name="aid">
                                                                    <option value="">请选择</option>
                                                                    {pg: foreach item=v from=$agent}
                                                                    <option value="{pg:$v.aid}" {pg: if $v.aid==$mer.aid}selected="selected"{pg:/if}>{pg:$v.uname}</option>
                                                                    {pg: /foreach}        
								</select>
							</label>
						</p>
