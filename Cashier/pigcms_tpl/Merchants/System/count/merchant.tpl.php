                                    ...
                                    <div id="dataselect" class="form-group" style="padding: 0 10px;">
                                        ...
                                        <div id="datepicker" class="input-daterange">
                                            ...
                                            <select name="mtype" style=" width: 120px; height: 30px; margin-bottom: 0px">
                                                ...
                                                <option value="1" {pg:if $getdata.mtype==1}selected="selected"{pg:/if}>特约商户</option>
                                                <option value="2" {pg:if $getdata.mtype==2}selected="selected"{pg:/if}>银行间连</option>
                                                <option value="3" {pg:if $getdata.mtype==3}selected="selected"{pg:/if}>金海哲</option>
                                            </select>
                                            <label class="font-noraml">商户名称</label>
                                            
                                            <input class="input form-control" type="text" name="username"  value="{pg: if (isset($getdata.username))}{pg:$getdata.username}{pg:/if}" placeholder="输入商户名称" style="width: 120px;border-radius: 7px;height: 30px; margin-bottom: 0px;border-radius: 3px;">
                                            <label class="font-noraml">选择日期</label>&nbsp;&nbsp;&nbsp;
                                            <input type="text" value="{pg: if (isset($getdata.start))} {pg: $getdata.start}{pg:else}{pg:$today}{pg:/if}" name="start" class="input-sm form-control" id="datestart" placeholder="开始时间" style=" margin-bottom: 0px; width:120px; height: 30px;border-radius: 3px">
                                            &nbsp;<span> 到 </span>&nbsp; 
                                            <input type="text" value="{pg: if (isset($getdata.end))} {pg: $getdata.end}{pg:else}{pg:$today}{pg:/if}" name="end" class="input-sm form-control" id="dateend" placeholder="结束时间" style=" margin-bottom: 0px; width: 120px;height: 30px;border-radius: 3px">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                {pg:if $v.mtype==3}
                                                    ...
                                                    <td>
                                                        <p>{pg:if !empty($v.wxsum.0.money)}{pg:$v.wxsum.0.money}{pg:else}0{pg:/if}</p>
                                                        <p>{pg:if !empty($v.qqsum.0.money)}{pg:$v.qqsum.0.money}{pg:else}0{pg:/if}</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:$v.wxsum.0.count}</p>
                                                        <p>{pg:$v.qqsum.0.count}</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:$v.commission*100}%</p>
                                                        <p>{pg:$v.qqcommission*100}%</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:if !empty($v.wxsum.0.income)}{pg:$v.wxsum.0.income}{pg:else}0{pg:/if}</p>
                                                        <p>{pg:if !empty($v.qqsum.0.income)}{pg:$v.qqsum.0.income}{pg:else}0{pg:/if}</p>
                                                    </td>
                                                {pg:else}
                                                    <td>
                                                        <p>微信</p>
                                                    ...
                                                    <td>
                                                    <p>{pg:if !empty($v.wxsum.0.money)}{pg:$v.wxsum.0.money}{pg:else}0{pg:/if}</p>
                                                    <p>{pg:if !empty($v.alisum.0.money)}{pg:$v.alisum.0.money}{pg:else}0{pg:/if}</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:$v.wxsum.0.count}</p>
                                                        <p>{pg:$v.alisum.0.count}</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:$v.commission*100}%</p>
                                                        <p>{pg:$v.alicommission*100}%</p>
                                                    </td>
                                                    <td>
                                                        <p>{pg:if !empty($v.wxsum.0.income)}{pg:$v.wxsum.0.income}{pg:else}0{pg:/if}</p>
                                                        <p>{pg:if !empty($v.alisum.0.income)}{pg:$v.alisum.0.income}{pg:else}0{pg:/if}</p>
                                                    </td>
                                                {pg:/if}
                                                

                                                <td><a href="/merchants.php?m=System&c=count&a=mdetail&mid={pg:$v.mid}"><button>查看</button></a></td>
                                            </tr>
                                             {pg:/foreach}
                                             {pg:else}
