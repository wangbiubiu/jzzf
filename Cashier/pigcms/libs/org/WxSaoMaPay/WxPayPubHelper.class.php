...
class Common_util_pub
{
	...
	public function postXmlSSLCurl($xml, $url, $second = 30)
	{
		...
		curl_setopt($ch, CURLOPT_SSLCERT, getcwd() . C('config.pay_weixin_client_cert'));
		curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
		curl_setopt($ch, CURLOPT_SSLKEY, getcwd() . C('config.pay_weixin_client_key'));
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
		$data = curl_exec($ch);
	...
	public function createXml()
	{
		try {
			if ($this->parameters['out_trade_no'] == NULL) {
			 ...
			 else if (($this->parameters['trade_type'] == 'JSAPI') && ($this->parameters['openid'] == NULL) && ($this->parameters['sub_openid'] == NULL)) {
				throw new SDKRuntimeException('统一支付接口中，缺少必填参数openid或sub_openid！trade_type为JSAPI时，openid或sub_openid为参数必传其一！' . '<br>');
			}
