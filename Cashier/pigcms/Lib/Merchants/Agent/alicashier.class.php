...
class alicashier_controller extends common_controller
{
	...
	public function aliRefund()
	{
		...
		$ret = $AliPayClass->aliRefund($orderArr['transaction_id'], $orderArr['goods_price'], $orderArr['order_id']);
	...
	public function alipay()
	{
		if (IS_POST) {
			...
			if ($response) {
				...
				$orderDb = M('cashier_order');
				$updatedata = array('paytime' => time(), 'ispay' => $response['ispay'], 'state' => 1, 'truename' => $response['buyer_logon_id'], 'openid' => $response['open_id'], 'transaction_id' => $response['trade_no'], 'extrainfo' => serialize($response));
