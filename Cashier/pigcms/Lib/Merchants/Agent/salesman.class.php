...
class salesman_controller extends common_controller {
    ...
    public function index() {
        ...
        if (!empty($_POST['username'])) {
            ...
            $where['aid'] = $this->aid;   
            $count = M('cashier_salesmans')->count($where);
            ...
            $salesmans = M('cashier_salesmans')->select($where,'id,username,account,phone,alicommission,commission',$limit);

 
        foreach ($salesmans as $k => &$v) {

            $v['wxrate'] = $v['commission'];
            $v['alirate'] = $v['alicommission'];
    ...
    public function createSaler () 
    {
        ...
        if (!empty($_POST['username'])) 
        {
            $addSaler = $this->clear_html($_POST);
            $addSaler['aid'] = $this->aid;
            $addSaler['salt'] = mt_rand(111111, 999999);
            $pwd = $addSaler['password'];
            if(!isEmail($addSaler['account'])){
                 $this->errorTip('请使用邮箱作账号', $_SERVER['HTTP_REFERER']);
                exit;
            }
            if (!isMobile($addSaler['phone']))
            {
                $this->errorTip('不是有效的手机号码', $_SERVER['HTTP_REFERER']);
                exit;
            }

            if (!empty($addSaler['password'])) 
            {
                $addSaler['password'] = $this->toPassword($addSaler['password'],$addSaler['salt']);
            }else{
                $this->errorTip('请填写密码', $_SERVER['HTTP_REFERER']);
                exit;
            }
            $addSaler['commission']=$addSaler['commission']/100;
            if (M('cashier_salesmans')->insert($addSaler)) {
                $url = $this->SiteUrl.'/salesman.php';
                bpBase::loadOrg('Email');
                $email = new Email();
                $subject = "重庆云极付有限公司业务员平台注册成功通知";//设置邮箱标题
                $address = $addSaler['account'];//需要发送的邮箱地址
                $content = <<<ETC
                 <div style="width: 80%; background: #FFFFFF;">
        			<h1 style="font-weight: normal; text-align: center; width: 100%; border-bottom: 1px solid #F3F3F3;">欢迎注册云极付业务员平台</h1>
...
ETC;
                ...
                $this->successTip('添加业务员账号成功', '?m=Agent&c=salesman&a=index');
                exit;
            }
        }
        include $this->showTpl(); 
    ...
    // 业务员管理
    public function mngSaler () {
       if(empty($_GET['sid'])) $this->errorTip('账号异常出错');

        $where['id'] = $_GET['sid'];
        $saler = M('cashier_salesmans')->get_one($where,'username,id,account,phone,commission,aid');

        $count = M('cashier_merchants')->count(array('sid'=>$where['id']));
        $page = new page($count,5);
        $limit = $page->firstRow . ',' . $page->listRows;
        $p = $page->show();
        $merchant = M('cashier_merchants')->get_all('mid,realname,username,commission,phone,tel,sid,company','',array('sid'=>$where['id']),'mid desc',$limit);


        foreach ($merchant as $k => &$v) {
           $v['storeNum'] = M('cashier_stores')->count(array('mid'=>$v['mid']));
        }
        include $this->showTpl();
    }
    ...
    public function getCashierBank () {
        if (IS_POST) {
            ...
            $result = M('cashier_salesmans')->get_var(array('id'=>$postdata['sid']),'bankandid');
            ...
            if  ($result) {
                ...
                echo json_encode(array('error'=>1,'url'=>$url));exit;
            }
        }
    ...
    public function setSaler () 
    {
        ...
        if (IS_GET) {
            ...
            $saler = M('cashier_salesmans')->get_one($where,'username,id,account,phone,commission,alicommission,cms_two,alicms_two,aid');
        ...
        if (IS_POST){
            ...
            $data = $this->clear_html($_POST);
            $setSaler['username'] = $data['username'];
            $setSaler['account']  = $data['account'];

            if (empty($data['id'])) $this->errorTpl('账号错误');
                $where['id'] = $data['id'];
            if ($data['changePwd']) {
                if ($data['password'] !=$data['password2']) $this->errorTip('两次输入密码不一致！');
                $salt = M('cashier_salesmans')->get_one($where,'salt');
                $setSaler['password'] = toPassword($data['password'],$salt);

            }

            if (!isMobile($data['phone'])) $this->errorTip('不是有效的手机号码');
                $setSaler['phone'] = $data['phone'];

            if (is_float($data['commission'])) $this->errorTip('分润比格式错误'); 
                $setSaler['commission'] = $data['commission']/100;
//
//            if (is_float($data['cms_two'])) $this->errorTip('分润比格式错误'); 
//                $setSaler['cms_two'] = $data['cms_two'];
//
//             if (is_float($data['alicommission'])) $this->errorTip('分润比格式错误'); 
//                $setSaler['alicommission'] = $data['alicommission'];
//
//            if (is_float($data['alicms_two'])) $this->errorTip('分润比格式错误'); 
//                $setSaler['alicms_two'] = $data['alicms_two'];

            if (M('cashier_salesmans')->update($setSaler,$where)){
                $this->successTip('账号修改成功','?m=agent&c=salesman&a=mngSaler&sid='.$data['id']);exit;
            }
        }
    ...
    public function delSaler () {
        ...
        if (IS_POST) {
            $data = $this->clear_html($_POST);
            $return = $this->_del(M('cashier_salesmans'), $data['id']);
    ...
    public function data2Excel () {
        //查询数据
        $salers = M('cashier_salesmans')->select(array('aid'=>$this->aid),'id,phone,aid,commission,username,account');
        $agent = M('cashier_agent')->get_one(array('aid'=>$this->aid),'uname');

        //组装数据Excel导出数据
        $data = array();
        foreach($salers as $key => $val){
            $data[$key]['id']      = ' '.$val['id'].' ';//商户id
            $data[$key]['account']  = $val['account'];//登录帐号
            $data[$key]['username'] = $val['username'];//名称   
            $data[$key]['commission']  = $val['commission'];// 佣金返点
            $data[$key]['phone']  = $val['phone'];//联系方式
        }
        $title = array('用户ID','登录帐号','用户名称','佣金返点','联系方式');
        $filename = '业务员:【'.$agent['uname'].'】.xls';
