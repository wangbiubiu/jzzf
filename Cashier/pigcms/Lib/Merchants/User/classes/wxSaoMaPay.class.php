...
class wxSaoMaPay
{
	...
	public function wxRefund($ordid, $wx_user, $mid,$reprice=null,$type = '')
	{
		...
		if (!empty($ordertmp)) {
            ...
            if(empty($reprice)){
                $reprice=$ordertmp['goods_price'];
            }
            $income=$ordertmp['goods_price']-$reprice;
			$input->SetTotal_fee($ordertmp['goods_price'] * 100);
			$input->SetRefund_fee($reprice * 100);
			...
			if (!empty($response)) {
				...
				if ($response['return_code'] == 'SUCCESS') {
					...
					$updatedata = array('refund' => 2,'income'=>$income, 'refundtext' => serialize($response));
