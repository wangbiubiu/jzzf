...
class merchant_controller extends common_controller
{
    ...
    public function merLists()
    {
        ...
        $sql = "SELECT m.mid,m.username,m.company,m.mtype,m.regTime,m.isopenwxpay,m.isopenalipay,a.uname FROM ".$this->tablepre . "cashier_merchants as m LEFT JOIN ".$this->tablepre . "cashier_agent AS a ON a.aid=m.aid where ".$wherestr ." ORDER BY m.mid DESC LIMIT " .$p->firstRow . ',' . $p->listRows;
        $sqlObj = new model();
        $merInfo = $sqlObj->selectBySql($sql);
        foreach($merInfo as &$v){
    ...
    public function pieces(){
        ...
        $pagebar = $p->show(2);
        //查询进件内容
        $sql = "SELECT r.id,r.status,m.mid,m.company,m.sub_merchant,m.realname,m.phone,r.wechat,r.alipay FROM ".$this->tablepre."cashier_merchants as m JOIN ".$this->tablepre."cashier_regist as r ON m.mid=r.mid where ".$wherestr." AND r.status > -1 ORDER BY m.mid desc, r.id desc LIMIT ".$p->firstRow.",".$p->listRows;
        ...
        foreach($rows as $key => $r){
            ...
            $array[$r['mid']]['sub_merchant'] = $r['sub_merchant'];
    ...
    public function config(){
        ...
        //查询进件状态
        $state = M('cashier_regist')->get_one(array('mid' => $mid,"wechat"=>""),'status');

        $mer = M('cashier_merchants')->get_one('mid='.$mid,'sub_merchant');
        $mtype = $mer['sub_merchant'];
        if($mtype == 1){
            ...
            $state1 = M('cashier_regist')->get_one(array('mid' => $mid),'status');
            $state2 = M('cashier_regist')->get_one(array('mid' => $mid),'status');
        }
        if (IS_POST) {
