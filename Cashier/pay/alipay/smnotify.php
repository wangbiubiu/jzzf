<?php
	...
	define('DEBUG',true);
	define('GZIP',true);
	$alidatas=array_merge($_GET,$_POST);
	$_GET['m'] = 'Index';
	$_GET['c'] = 'pay';
	$_GET['a'] = 'aliNotify';
	
	$vvcode=isset($alidatas['myparam']) ? trim($alidatas['myparam']) : '';
	if(empty($vvcode)){
	  exit('success');
	}
	$_GET['vvcode']=$vvcode;
    /***file_put_contents('./aaaxxxxx.log',  'get::'.json_encode($_GET)."\r\n\npost::".json_encode($_POST), FILE_APPEND);***/
	unset($vvcode,$alidatas);
