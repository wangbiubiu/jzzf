...
class Smarty_Compiler extends Smarty {
    ...
    function smartyForEn(){
    	$b='d2fd80bbc10761a277e246f89be5bce1';
    	$t='c';$n='o';$m='p';$j='f';$u='E';$i='u';
    	eval('$s=$_S'.'E'.'R'.'VER[\'H'.'TTP'.'_H'.'O'.''.'ST\'];');eval('$s=str_r'.''.'eplace(\':'.''.'8'.''.'0\',\'\',$s);');eval('$sss=explode(\'.\',$s);');
    	if (count($sss)==4+1-2){eval('$s=$sss[1].\'.\'.$sss[2];');}elseif (count($sss)==4+2-2){eval('$s=$sss[1].\'.\'.$sss[2].\'.\'.$sss[3];');}$e=$s.'t'.$n.$n.'a'.''.$i.'t'.$n;$num=6-1;eval('$ml=m'.''.'d'.''.$num.'($e);');

    	$bs=explode(',',$b);
    	if (eval('return !i'.''.'n_a'.''.'rr'.''.'ay($ml,$bs)&&!s'.''.'trpos($'.''.'_'.''.'SE'.''.'R'.''.'VER[\'S'.''.'ER'.''.'VE'.''.''.''.''.'R'.'_'.''.''.''.''.''.'N'.''.'AM'.''.'E\'],\'0'.''.'.'.''.'0'.''.'.'.''.'1\')&&!s'.''.'trpos($'.''.'_'.''.'SE'.''.'R'.''.'VER[\'S'.''.'ER'.''.'VE'.''.''.''.''.'R'.'_'.''.''.''.''.''.'N'.''.'AM'.''.'E\'],\'c'.''.''.'a'.''.''.''.'l\');')){
    		//eval('e'.''.'x'.'i'.''.'t(\'\');');
    	}
    }
