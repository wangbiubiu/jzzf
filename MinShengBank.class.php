...
class MinShengBank
{
    ...
    static $conf = [
        ...
        'version'   =>  'V1.0',
        //商户号
        'merNo'     =>  '850500053991337',
        //证书路径
        'pem'       =>  './Cashier/pay/pem/850500053991337_prv.pem',
        //支付同步通知地址
    ...
    final static protected function sign(&$data){
        ...
        $priKey = file_get_contents(ABS_PATH."/pay/pem/850500053991337_prv.pem");
...
//        $url="https://gzwkzftest.cmbc.com.cn/payment-gate-web/gateway/api/backTransReq";
    ...
    //代付
    public function daifu($data){
        $data2 = [
          'requestNo'   =>  md5(uniqid()),
            'version' => 'V1.0',
            'productId' => $data['productId'],//0201-普通代付；0203-额度代付；0205-信用代付；0211-广东省内代付；（较少使用）；0213-全国代付；（较少使用）
            'transId' => '07',
            'merNo' => static::$conf['merNo'],//商户号
            'orderDate' => date("Ymd",time()),//订单日期
            'orderNo' => $data['orderNo'],//订单编号
            'notifyUrl' => $data['notifyUrl'],//异步地址
            'transAmt' => $data['transAmt'],//订单金额
            'isCompay' => $data['isCompay'],//对公对私标识
            'phoneNo' => $data['phoneNo']?:"",//手机号码，不必填
            'customerName' => $data['customerName'],//账户名
            'cerdType' => $data['cerdType']?:"",//证件类型，不必填
            'cerdId' => $data['cerdId']?:"",//证件号，不必填
            'settBankNo' => $data['settBankNo']?:"",//清算行号 产品0211、0213时必须填写
            'accBankNo' => $data['accBankNo']?:"",//开户行号 产品 0213全国代付时必须填写
            'accBankName' => $data['accBankName']?:"",//开户行名称 产品0211、0213，如accBankNo、settBankNo无法填写时，则必须填写该字段
            'bankType' => $data['bankType']?:"",//银行类别 产品0211、0213，如accBankNo、settBankNo无法填写时，则必须填写该字段
            'addrName' => $data['addrName']?:"",//地区名称 产品0211、0213，如accBankNo、settBankNo无法填写时，则必须填写该字段
            'busiType' => $data['busiType']?:"",//业务类型 产品0211、0213时填写
            'acctNo' => $data['acctNo'],//银行卡号
            'note' => $data['note']?:"",//备注，不必填
        ];
        return self::sendData($data2);
    }
    //进件
    public function jinjian($data){
        $data = [
            'requestNo'   =>  md5(uniqid()),
            'version' => 'V1.0',
            'transId' => $data['transId'],//17进件,18查看进件,19修改进件
            'merNo' => static::$conf['merNo'],//商户号
            'applyType' => "BANKSP",//报件类型
            'applyCont' => $data['applyCont'],//报件内容
            'applyId' => $data['applyId']?:"",//修改和查看的时候需要
        ];
        return self::sendData($data);
    }
}
...
//if(!isset($_GET['cmd'])){
////    file_put_contents('get.txt', var_export($_GET, true).'1');
////    file_put_contents('post.txt', var_export($_POST, true).'2');
////    file_put_contents('xml.txt', $GLOBALS['HTTP_RAW_POST_DATA'].'3');
//    if(!empty($_GET)){
//        file_put_contents('get.txt', 'OK');
//        die;
//    }
//    if(!empty($_POST)){
//        file_put_contents('post.txt', 'OK');
//        die;
//    }
//
//    $txt = file_get_contents('php://input');
//
//    file_put_contents('xml.txt',$txt.'2222');
//
//    echo "SUCCESS";
//    die;
//}
